...
legand = Token:match("(%d+)")
redis:set(legand..":VERSION",1)
redis:set(legand..":SUDO_ID:",SUDO_USER)
redis:set(legand..":DataCenter:",'German')
redis:set(legand..":UserNameBot:",BOT_User)
redis:set(legand..":NameBot:",BOT_NAME)
redis:hset(legand..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3legand_INSTALL","Yes")
info = {}
...
legand = Token:match("(%d+)")
our_id = tonumber(legand)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(legand..":UserNameBot:")
SUDO_ID = tonumber(redis:get(legand..":SUDO_ID:"))
SUDO_USER = redis:hgetall(legand..'username:'..SUDO_ID).username
version = redis:get(legand..":VERSION")
DataCenter = redis:get(legand..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..legand..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(legand..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(legand..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(legand..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 835551362 then 
...
msg.Rank = 1
elseif redis:sismember(legand..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(legand..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(legand..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(legand..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(legand..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(legand..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(legand..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(legand..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(legand..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(legand..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(legand..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(legand..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس ليجاند ⇐ @SUORSlegand
...
if msg.GroupActive and redis:get(legand..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(legand..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(legand..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(legand..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.legand then 
if msg.text and ScriptFile.ilegand then
for k, legand in pairs(ScriptFile.legand) do
local SearchText = msg.text:match(legand)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.ilegand(msg,{msg.text:match(legand)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",legand.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dlegand then
if ScriptFile.dlegand(msg) == false then
return false
end
...
if Plug.legand then 
if msg.text and Plug.ilegand then
for k, legand in pairs(Plug.legand) do
local SearchText = msg.text:match(legand)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(legand..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(legand..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.ilegand(msg,{msg.text:match(legand)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",legand.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
if Plug.dlegand then
Plug.dlegand(msg)
print("\27[1;35m¦Msg_IN_Process : \27[1;32"..name.."\27[0m")
end
else
print("The File "..name.." Not Runing in The Source legand")
end 

end
else
print("The File Script.lua Not Runing in The Source legand")
...
function tdcli_update_callback(data)
	...
	redis:srem(legand..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/jack1eljoker1/legand/main/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(legand..":SUDO_ID:")
redis:hset(legand..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(legand..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(legand..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(legand..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(legand..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(legand..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(legand..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(legand..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(legand..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(legand..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(legand..'group:add'..data.chat_.id_) then
	redis:set(legand..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(legand..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(legand..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(legand..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	legand..'group:name'..k,IDS.Title,
	legand..'num_msg_legand'..k,5,
	legand..'group:add'..k,true,
	legand..'lock_link'..k,true,
	legand..'lock_spam'..k,true,
	legand..'lock_webpage'..k,true,
	legand..'lock_markdown'..k,true,
	legand..'lock_flood'..k,true,
	legand..'lock_bots'..k,true,
	legand..'mute_forward'..k,true,
	legand..'mute_contact'..k,true,
	legand..'mute_document'..k,true,
	legand..'mute_inline'..k,true,
	legand..'lock_username'..k,true,
	legand..'replay'..k,true
	)
	redis:sadd(legand..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(legand..'username:'..ID,'username',user)
	redis:sadd(legand..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(legand..'username:'..ID,'username',user)
	redis:sadd(legand..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(legand..'username:'..ID,'username',user)
	redis:sadd(legand..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(legand..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(legand..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(legand..'username:'..data.user_.id_).username
	...
	redis:hset(legand..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
